...
| **Document status** | **Rev. 4 (Final)** — revised Sept 7, 2026 (Week 1, Day 1) following an independent procedural review of Rev. 3. Rev. 2 closed requirement gaps; Rev. 3 closed architecture gaps; Rev. 4 closes the gap in *how the remaining human decisions actually get made*, and adds one honesty note about the review chain itself. Changes marked **[Rev. 4]** inline, on top of the existing **[Rev. 3]**, **[Rev. 2]**, and **[Recommended]** conventions. |
| **Review chain [Rev. 4]** | Rev. 1 (from the source workbook) → Rev. 2, a requirements review (`Review_Memo.md`) → Rev. 3, an architecture review (`Opus5_Change_Notes.md`) → **Rev. 4, a procedural review (this document; `Final_Change_Notes.md`)**. Each pass was made by a reviewer of the same general kind as the one before it. None of the four substitutes for a human domain expert or the stakeholders named throughout §6 and §9c — that's why those sections exist, and why "Final" describes the document, not the plan's validation status. |
| **Companion documents** | `Opus5_TRD.md` → superseded by `Final_TRD.md` — technical architecture, data specs, and model/interface requirements · `Final_Change_Notes.md` **[Rev. 4]** — what changed in Rev. 4 and why · `Opus5_Change_Notes.md` — the Rev. 2 → Rev. 3 record · `Review_Memo.md` — the Rev. 1 → Rev. 2 record |
| **Timing note** | Every deadline this document introduces sits ahead of the work it governs — none of it is retrofitted onto work already done. |
...
> **[Rev. 4] "Defensible" still means "confirmed," not "recommended."** The ranking rule in TRD §9.2 is well-reasoned, but it encodes a domain judgement (a leak outranks corrosion at equal confidence) that no document revision can validate on its own. §9c turns "needs stakeholder confirmation" from a recurring footnote into a scheduled Week-1 conversation with a named owner and a recorded outcome.
...
| **[Rev. 4]** Any additional validation, metadata, or test layer beyond what §9 (TRD) already specifies | Rev. 3 established real rigor (a validated contract, versioned artifacts, a risk-model table, two smoke assertions) at a real cost against a 4–6 hr/week budget. Extending that rigor further is out of scope for this PoC unless it goes through the same one-paragraph decision process (TRD §11, "Pattern discipline") |
...
> **[Rev. 4]** All three of the above are checkable — a smoke test either passes or it doesn't. The item that isn't yet checkable is whether the *band table itself* is right (§6 assumption 9). That distinction matters: this PoC can be built exactly to spec and still be shipping an unconfirmed default. §9c is where that gets closed, not another revision of this document.
...
| 3 | **Time budget** — ~4–6 hrs/week/person, possibly the same two people already committed to a parallel "LeRobot" pilot | **Yes** | If the same two people run both pilots, combined load could reach ~8–12 hrs/week across two initiatives on top of client work. **[Rev. 4]** Now has a defined fallback if unconfirmed — see §9d |
...
> **[Rev. 4] These don't get resolved by another document revision.** Items 2, 3, 7, 8, 9, and 10 have now sat across multiple revisions with a recommended default attached and no recorded answer. §9c bundles all six into one Week-1 conversation with a named owner and a written outcome per item. The goal isn't a better default — every item here already has one — it's making sure someone actually said yes, no, or "not yet, ask me again on [date]" before Week 3 and Week 6 commitments are made against an assumption nobody confirmed.
...
| Leadership / stakeholders | — | Review Checkpoint 1 (Wk 5) and final submission (Wk 8); confirm §6 assumptions at the Week 1 sync (§9c) |
...
|---|---|---|---|
| 1 | Onboarding | Both understand the use case; both verified real dataset families (RGB corrosion, OGI/methane) downloaded/vetted. **[Rev. 3]** Model-artifact handoff convention and data-persistence location agreed and written down (TRD §8.5, §4.8) — both are one-paragraph decisions that block Week 6 if deferred. **[Rev. 4]** Consolidated decision sync held (§9c); resourcing gate (§9d) triggered if item 3 is unconfirmed | |
...
| 3 | Independent work | First simulation extension (Anshuman) + both baseline classifiers (Shashwat) in place. **[Rev. 3]** Risk model (TRD §9.2) decided *before* training starts, alongside the taxonomy decision it depends on — both should already be confirmed from the Week 1 sync (§9c) | |
...
## 9c. Consolidated Week 1 Decision Sync **[Rev. 4]**

Six items across this document carry a recommended default and an unresolved "needs confirmation" — spread across §6, §9a, and TRD Appendix B, each with its own deadline (end of Week 1, before Week 3, before Week 4). Individually, each is easy to postpone one more week. Collectively, that's how a PoC ships six unconfirmed assumptions as if they were decisions.

**Rule:** one scheduled conversation, Week 1 — Day 2 or 3, after initial dataset/tooling setup but before any Week 2 commitment is made against these assumptions. 30–45 minutes. Every item gets a named owner and a written outcome: confirmed, adjusted, or explicitly deferred with a new date. Not a status update — a decision record, kept with the project files.

| # | Item | Ships silently as | Owner |
|---|---|---|---|
| 1 | Physical drone / field access | Confirmed already — simulation only (§6, item 2) | Project owner |
| 2 | Resourcing conflict — parallel LeRobot commitment | **Unconfirmed** — triggers the fallback in §9d if still open at end of Week 1 | Project owner |
| 3 | Minimum quality bar | Majority-class baseline + F1 ≥ 0.65 (§5) | Stakeholders |
| 4 | RGB taxonomy | Binary `corrosion`/`no_corrosion` for MVP (§6, item 8) | Stakeholders |
| 5 | Risk-model bands | Leak outranks corrosion at equal confidence (TRD §9.2) | Stakeholders / domain |
| 6 | `recommended_action` wording | Illustrative, engineer-written (§6, item 10) | Stakeholders / domain |

A default that ships because nobody said otherwise is not the same as a default that was confirmed. The point of this sync is that the difference is on the record before Week 3 classifier work and Week 6 work-order wiring commit to these assumptions in code.

## 9d. Resourcing Confirmation Gate **[Rev. 4]**

The license audit (TRD TR-4.1) is a Week 1 go/no-go gate with a defined fallback: unresolved by end of Week 1, drop the Kaggle set and proceed without it. The resourcing conflict — the same two people possibly running this PoC alongside a parallel "LeRobot" pilot — has sat in every revision as an open risk with **no** fallback. That's inconsistent: it's the single largest unquantified risk in the plan (§10), and it has been treated more casually than a dataset license.

**Rule:** resourcing is confirmed at the Week 1 sync (§9c). If it is *not* confirmed by end of Week 1:

- Treat effective capacity as the lower bound already named in §6, assumption 3 — roughly 8–12 hrs/week combined across both initiatives, not 4–6 hrs/week/person dedicated solely to this one — until told otherwise.
- Re-baseline the Week 3 and Week 6–7 pressure points (TRD §13a) against that lower bound at the Week 1 stakeholder review, rather than discovering the shortfall when Checkpoint 1 is already at risk.

This does not resolve the resourcing question — only the project owner's actual answer does. It gives the plan a stated behavior for the case where that answer doesn't arrive on time, instead of silently assuming the best case.

## 10. Risks & Open Questions

- **Resourcing conflict:** the same two people may already be committed to a parallel "LeRobot" pilot at a similar part-time cadence — combined load could reach ~8–12 hrs/week if confirmed. *(Needs confirmation, §6.)* **[Rev. 4]** Now has a defined fallback if unconfirmed by end of Week 1 — see §9d.
...
- **[Rev. 4] The review chain is not an independent check.** Rev. 2, Rev. 3, and this document were each produced by a reviewer examining the pass before it — none is a substitute for the human stakeholder or domain sign-off named throughout §6 and §9c. A document revision can find a missing rule or an untyped field; it cannot confirm that a domain judgement is the right one.
...
- **[Rev. 4]** A written record of the Week 1 decision sync (§9c) — six confirmed-or-deferred outcomes, not six open questions carried silently into Week 3
...
| Decision sync **[Rev. 4]** | The single Week 1 conversation (§9c) where every outstanding "needs stakeholder confirmation" item gets a named owner and a recorded outcome, instead of a scattered deadline in three different documents |
...
- `Final_TRD.md` **[Rev. 4]** — companion technical requirements document, superseding `Opus5_TRD.md`.
- `Opus5_TRD.md` — the Rev. 3 technical requirements document, retained for history.
- `Opus5_Change_Notes.md` — what changed between Rev. 2 and Rev. 3, and why.
- `Final_Change_Notes.md` **[Rev. 4]** — what changed between Rev. 3 and Rev. 4, and why.
...
- `PRD.md` / `TRD.md` — the Rev. 2 documents this revision chain supersedes, retained for history.
