...
### *Companion to `Opus5_PRD.md` — derived from `Pipeline_Integrity_Timeline.xlsx`*
...
| **Companion doc** | `Opus5_PRD.md` — product framing, stakeholders, timeline, risks |
...
| **Status** | **Rev. 3** — revised Sept 7, 2026 (Week 1, Day 1) following an architecture-level review of Rev. 2. Changes marked **[Rev. 3]** inline, on top of the existing **[Rev. 2]** and **[Recommended]** conventions. Dataset facts in §3 were independently re-verified against primary sources on Sept 2, 2026. See `Review_Memo.md` for the Rev. 1 → Rev. 2 rationale and `Opus5_Change_Notes.md` for the Rev. 2 → Rev. 3 rationale. |
| **What Rev. 3 addresses** | Rev. 2 closed *requirement* gaps (leakage, taxonomy, licensing, success bar). Rev. 3 closes *architecture* gaps: the interface seam between the two owners, the undefined risk model behind the headline output, model-artifact handoff, and data persistence. |
...
│   ├── Opus5_PRD.md
│   ├── Opus5_TRD.md
│   └── Opus5_Change_Notes.md
├── data/
│   ├── raw/                  # untouched downloads per §3
│   ├── processed/            # cleaned/standardized — Drive-mounted, TR-4.8
│   ├── manifest.json         # counts, class breakdown, split assignment [Rev. 3]
...
│   └── visualize.py          # TR-6.5
├── models/
│   ├── interfaces.py         # §5 contract as validated code [Rev. 3]
│   ├── rgb_corrosion/
│   │   ├── train.py
│   │   ├── predict.py        # shared-interface wrapper [Rev. 3]
│   │   └── artifacts/<ver>/  # weights, config, metrics, VERSION [Rev. 3]
│   ├── ogi_methane/
│   │   ├── train.py
│   │   ├── predict.py
│   │   └── artifacts/<ver>/
...
│   ├── risk_model.py         # §9.2 band table [Rev. 3]
...
> **[Rev. 3] Net effect on the schedule.** Rev. 3 adds roughly 3–4 hours of Week 1–2 decision work (contract as code, artifact convention, risk-model table, persistence setup) and removes a larger, less predictable amount of Week 6–7 rework and negotiation. It is a deliberate shift of effort earlier, into the weeks where the schedule has the most slack and the decisions are cheapest to make.
...
- **[Rev. 2] Week 1 go/no-go gate:** license audit (TR-4.1) explicitly signed off — or its fallback triggered — before any dataset is used for training.
...
- A full CI/CD suite is disproportionate at this scale — a single `run_e2e_demo.py` smoke-test script is recommended, with the two Rev. 3 assertions added to it rather than a new framework introduced.
...
|---|---|
| 1 | Datasets downloaded/vetted (counts re-confirmed per §3.4); licenses checked; shared tooling set up. **[Rev. 2] Go/no-go:** license audit (TR-4.1) signed off or fallback triggered before Week 2 work begins. **[Rev. 3]** Artifact-handoff convention (TR-8.5) and persistence location (TR-4.8) written down |
...
| 3 | Simulator extended (multi-route + injection); both baseline classifiers built; **risk model table decided (§9.2)** |
...
| 8 | **Final** — full E2E PoC demo; limitations (including Rev. 3 honesty items) + Phase 2 roadmap documented |
